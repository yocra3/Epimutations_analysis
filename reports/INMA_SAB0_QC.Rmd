...
ind.res.df <- Reduce(rbind, res_ind) %>%
  mutate(method = rep(names(res_ind), sapply(res_ind, nrow))) %>%
  left_join(colData(ind.Esteller) %>% data.frame() %>% select(Sample_Name, idnum, Batch) %>% mutate(sample = Sample_Name), by = "sample") %>%
   mutate(Normalization = "Independent")

comb.res.df <- Reduce(rbind, res_comb) %>%
  mutate(method = rep(names(res_comb), sapply(res_comb, nrow))) %>%
  left_join(colData(comb.gset.Esteller) %>% data.frame() %>% select(Sample_Name, idnum, Batch) %>% mutate(sample = Sample_Name), by = "sample") %>%
...
When datasets are independently normalized, we observe a higher number of epimutations in the replicate from the external batch (MeDALL) than in the replicate form the reference batch (Esteller). This effect is not so obvious when both datasets are normalized jointly.
